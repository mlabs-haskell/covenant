...
import Covenant.Constant (AConstant (AUnit, ABoolean, AnInteger, AByteString, AString))
import Covenant.DeBruijn (DeBruijn, asInt)
import Covenant.Index (Count, Index, intCount, intIndex)
import Covenant.Internal.Term (CovenantTypeError, Id (Id), Ref (AnArg, AnId), Arg (..), ValNodeInfo (LitInternal, AppInternal, ThunkInternal, CataInternal, DataConstructorInternal, MatchInternal), CompNodeInfo (Builtin1Internal, Builtin2Internal, Builtin3Internal, Builtin6Internal, LamInternal, ForceInternal), ASGNode (ACompNode, AValNode, AnError))
import Covenant.Prim (OneArgFunc, SixArgFunc, ThreeArgFunc, TwoArgFunc)
import Covenant.Type
  ( AbstractTy,
    BuiltinFlatT (BLS12_381_G1_ElementT, BLS12_381_G2_ElementT, BLS12_381_MlResultT, UnitT, BoolT, IntegerT, StringT, ByteStringT),
    CompT,
    CompTBody,
    ...
    PlutusDataConstructor (PlutusI, PlutusB, PlutusConstr, PlutusList, PlutusMap),
...
import Data.Aeson.Types (Parser, toJSON1, Object, Key, withObject, Value (Object, String, Array), withText, typeMismatch, prependFailure, Array)
...
import Covenant.Internal.Type (ConstructorName(ConstructorName), DataDeclaration (OpaqueData), ValT (ThunkT, BuiltinFlat), AbstractTy (BoundAt), CompT (CompT), CompTBody (CompTBody))
import qualified Data.Vector as Vector
import Covenant.Internal.Strategy (InternalStrategy (InternalAssocMapStrat, InternalPairStrat, InternalListStrat, InternalOpaqueStrat))
...
import Optics.Core (review, preview)
...
decodeVersion = liftParser . withObject "Version" (\obj -> do
...
decodeConstructorName = liftParser . withText "ConstructorName" (\nm -> do
...
decodeConstructor = withObject' "Constructor" $ \obj -> do
  ...
  ctorArgs <- lookupAndParse' obj "constructorArgs"
...
decodePlutusDataStrategy
  = caseOnTag [ "EnumData" :=> constM EnumData
...
decodeInternalStrategy
  = caseOnTag [ "InternalListStrat" :=> constM InternalListStrat
...
decodePlutusDataConstructor
  = caseOnTag [ "PlutusI" :=> constM PlutusI
...
decodeDataDeclarationAbstractTy = caseOnTag [ "DataDeclaration" :=> goDataDecl
  ...
  where
    ...
    goOpaqueData obj = do
      ...
      dtName <- lookupAndParse' fieldsObj "datatypeName" decodeTyName
      plutusCtors <- lookupAndParse' fieldsObj "opaquePlutusConstructors"
...
decodeRef
  = caseOnTag [ "AnArg" :=> withFields (withIndex 0 (fmap AnArg . decodeArg ))
...
encodeValNodeInfo = \case
  ...
  DataConstructorInternal tn cn args ->
    taggedFields "DataConstructorInternal" [encodeTyName tn
...
decodeValNodeInfo
  = caseOnTag [ "LitInternal" :=> withField0 (fmap LitInternal . decodeAConstant)
              , "AppInternal" :=> withFields (\fieldsArr -> do
              ...
              , "MatchInternal" :=> withFields (\fieldsArr -> do
...
decodeCompNodeInfo
  = caseOnTag [ "Builtin1Internal" :=> withField0 (fmap Builtin1Internal . decodeOneArgFunc)
...
decodeASGNode
  = caseOnTag [ "ACompNode" :=> withFields $ \fields -> do
...
encodeValTAbstractTy = \case
  ...
  Datatype tn args -> pairs $ pair "tag" "Datatype"
...
decodeValTAbstractTy
  = caseOnTag [ "Abstraction" :=> withField0 (fmap Abstraction . decodeAbstractTy)
