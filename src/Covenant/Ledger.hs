...
plutusData = mkDecl $ Decl "Data" count0 [
...
credential = mkDecl $ Decl "Credential" count0  [
...
stakingCredential = mkDecl $ Decl "StakingCredential" count0 [
...
address = mkDecl $ Decl "Address" count0 [
...
interval = mkDecl $ Decl "Interval" count1 [Ctor "Interval" [
    tycon "LowerBound" [a],
    tycon "UpperBound" [a]
  ]] (PlutusData ConstrData)
...
assocMap = mkDecl $ Decl "Map" count2
...
value = mkSimpleNewtype "Value"
  (tycon "Map" [tycon "CurrencySymbol" [],
...
rational = mkDecl $ Decl "Rational" count0 [
...
mintValue = mkDecl $ Decl "MintValue" count0 [
  Ctor "UnsafeMintValue" [tycon "Map" [tycon "CurrencySymbol" [],
                tycon "Map" [tycon "TokenName" [], BuiltinFlat IntegerT]]]
  ] (PlutusData NewtypeData)
...
outputDatum = mkDecl $ Decl "OutputDatum" count0
  [ Ctor "NoOutputDatum" []
...
txInInfo = mkDecl $ Decl "TxInInfo" count0
  [Ctor "TxInInfo"
    [ tycon "TxOutRef" []
    , tycon "TxOut" []
    ]
...
delegatee = mkDecl $ Decl "Delegatee" count0
...
txCert = mkDecl $ Decl "TxCert" count0
...
voter = mkDecl $ Decl "Voter" count0
...
governanceActionId = mkDecl $ Decl "GovernanceActionId" count0
  [ Ctor "GovernanceActionId"
    [ tycon "TxId" []
...
committee = mkDecl $ Decl "Committee" count0
  [ Ctor "Committee"
...
protocolVersion = mkDecl $ Decl "ProtocolVersion" count0
  [ Ctor "ProtocolVersion" [BuiltinFlat IntegerT, BuiltinFlat IntegerT]
  ] (PlutusData ConstrData)

-- GovernanceAction (v3)
governanceAction :: DataDeclaration AbstractTy
governanceAction = mkDecl $ Decl "GovernanceAction" count0
  [ Ctor "ParameterChange" [tycon "Maybe" [tycon "GovernanceActionId" []]
                           , tycon "ChangedParameters" []
                           , tycon "Maybe" [tycon "ScriptHash" []]]
  , Ctor "HardForkInitiation" [tycon "Maybe" [tycon "GovernanceActionId" []]
                              , tycon "ProtocolVersion" []]
  , Ctor "TreasuryWithdrawals" [tycon "Map" [tycon "Credential" [], tycon "Lovelace" []]
                               , tycon "Maybe" [tycon "ScriptHash" []]]
  , Ctor "NoConfidence" [tycon "Maybe" [tycon "GovernanceActionId" []]]
  , Ctor "UpdateCommittee" [tycon "Maybe" [tycon "GovernanceActionId" []]
...
proposalProcedure = mkDecl $ Decl "ProposalProcedure" count0
  [ Ctor "ProposalProcedure"
    [ tycon "Lovelace" []
    , tycon "Credential" []
...
scriptPurpose = mkDecl $ Decl "ScriptPurpose" count0
...
scriptInfo = mkDecl $ Decl "ScriptInfo" count0
...
txInfo = mkDecl $ Decl "TxInfo" count0
  [ Ctor "TxInfo"
...
scriptContext = mkDecl $ Decl "ScriptContext" count0
  [ Ctor "ScriptContext"
    [ tycon "TxInfo" []
    , tycon "Redeemer" []
    , tycon "ScriptInfo" []
